...
| Add an Agent-loop tool | `/add-loop-tool` | `src/agent-loop/loop-tools.ts` + `loop-component-registry.tsx` | "Add a tool that calculates ROI for a given cost and revenue." |
| Add a new skill (slash command) | `/add-skill` | `.claude/commands/` | "Add a /smoke skill that starts the app and checks the tabs load." |
| Add a plan→approve→act action | (edit the `extend` agent + `planCard`/`planResult`) | `src/lessons/ExtendDemo.tsx` + the `extend` agent in `server.ts` | "Make 'archive my old accounts' go through a plan I approve first." |
